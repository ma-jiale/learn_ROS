...
def twist_talker(linear_x, angular_z):
    pub = rospy.Publisher('/turtle1/cmd_vel', Twist, queue_size=10) # 声明一个Publisher对象表示该节点正在使用String消息类型发布到chatter话题
    myTwist = Twist()
...
def joy_listener():
    ...
    rospy.init_node('use_joy_ctr_turtle', anonymous=True)
